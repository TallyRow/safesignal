...
function reduceToNameMessage(value: unknown): SerializedErrorNode {
...
/**
 * Pure, throw-free deep-copy walker: applies `transform` to every node
 * (the payload itself plus all causes + members, recursively), returning a
 * new structure. Operates only on already-extracted plain data — never on a
 * raw `Error` — so it cannot invoke consumer getters. Callers overwrite the
 * top-level `name`/`message`/`stack` afterwards with their existing flat
 * handling, so transforming them here is harmless.
 */
export function mapErrorNodes<T extends ErrorInfo | SerializedErrorNode>(
  node: T,
  ...
  if (next.causes !== undefined) {
    next.causes = next.causes.map((child) => mapErrorNodes(child, transform));
  }
  if (next.members !== undefined) {
    next.members = next.members.map((child) => mapErrorNodes(child, transform));
  }
  return next;
}
...
export function serializeError(
  ...
  const info: ErrorInfo = reduceToNameMessage(value);
  ...
  if (isErrorLike(value)) {
    ...
    captureCauses(info, value, limits, budget);
    captureMembers(info, value, limits, budget);
    captureFields(info, value, limits, budget);
  }
  ...
  if (budget.guardedFailure !== undefined && onInternalError !== undefined) {
    safeNotify(
      ...
      wrapAsPackageError(
        ...
        'a property accessor threw during deep error serialization',
 ...
 * Chain entries never carry their own `causes`, but an AggregateError in a
 * chain keeps its members (and fields) on the chain entry.
 */
function captureCauses(
  ...
  let truncated = false;
  let cursor: unknown = safeGet(source, 'cause');
...
/**
 * US2 / ES-4..ES-5: capture `source`'s aggregate members (a structural
 * array-valued `errors` property) onto `target.members`, original order.
 * Members recurse depth-first under the shared node budget. Clipping by
 * `maxMembers` or the budget records the original count in `membersTotal`.
 * Self-referential aggregates terminate via the budget.
 */
 ...
 * standard `name`/`message`/`stack`/`cause`/`errors` keys) onto
 * `target.fields`, value-filtered and clipped to `maxFields` with
 * `fieldsTruncated`. The sole prototype special case is `DOMException`'s
 * legacy numeric `code` (a prototype getter that generic own-property
 * capture cannot see). Hostile getters are skipped, recorded once on the
 * budget state, and reported by `serializeError` (US3.4).
 */
