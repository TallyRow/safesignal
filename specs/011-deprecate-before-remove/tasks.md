...
- [X] T017 [P] **Named, time-bound follow-up** (Principle X): filed as GitHub issue **#35** "Automated structural API compatibility classification" capturing the plan.md Complexity Tracking boundary (auto-distinguish compatible widenings from incompatible signature changes so the `surface-allow.json` override can shrink), with a stated deadline of **2026-09-01**; referenced from `plan.md` Complexity Tracking.
