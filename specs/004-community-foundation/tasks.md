...
- [X] T001 Capture pre-feature test-suite baseline: run `npm test` from repo root. Record the headline counts (test files / tests passing / tests todo / tests failing / unhandled errors) into a new file `specs/004-community-foundation/baselines.md` under a "## Pre-feature baseline" section
...
- [X] T011 [P] [US2] Create `.gitlab/issue_templates/Bug.md` per `data-model.md` § ".gitlab/issue_templates/Bug.md (NEW)". Required sections: Steps to reproduce, Expected behavior, Actual behavior, Package version, Browser/runtime, Minimal reproduction, Additional context (maps FR-023 / R-007)
