...
- [ ] T006 [US1] Relocate the migration block from `README.md`'s current position (directly under H1 as `## Renamed from \`frontend-logging-sdk\``) to a new `## Migration history` section deeper in the README (placed before `## Where to learn more`). Replace the H2 heading text from `Renamed from \`frontend-logging-sdk\`` to `Migration history`. ADD a 1–2 line intro paragraph above the relocated body explaining what the section contains. Body bytes MUST be identical to the source per `contracts/migration-note-preservation.md` (maps R-009 / FR-028 / SC-007)
- [ ] T007 [US1] Add a `## Project resources` section to `README.md` (placed between `## Examples` and `## Where to learn more`, or merged into the existing `## Where to learn more`). The section MUST link to all of: `CONTRIBUTING.md`, `SECURITY.md`, `CODE_OF_CONDUCT.md`, `GOVERNANCE.md`, `LICENSE`. (These files don't exist yet — they land in US2/US3/US4; the links are placeholders that resolve once those phases complete) (maps FR-030 / R-011)
...
- [ ] T022 Write `specs/004-community-foundation/checklists/final-review.md` recording: (a) the four contracts' PASS / FAIL outcomes, (b) pre-feature vs post-feature test-suite baseline numbers, (c) one-line acceptance statement, (d) confirmation that no source/test/dependency changes shipped, (e) any open follow-ups (e.g., the `tallyrow.com` mail aliases needing maintainer-side DNS/MX setup before contacts go live)
