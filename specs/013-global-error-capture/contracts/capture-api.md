...
| **CAP-3** | **Fail-closed**: the emitted event passes the same sanitize → URL-scrub → redact → guard pipeline; a secret-shaped value in the message/stack/reason is redacted before any transport sees it; a redaction failure drops the event. | FR-004 / SC-003 |
