...
- [ ] T002 **Maintainer**: rename GitLab default branch from `master` to `main` via Settings → Repository → Default branch. After save, verify with `git ls-remote --symref origin HEAD | head -1` (should print `ref: refs/heads/main`). GitLab automatically creates a `master` → `main` alias post-rename — **DO NOT actively disable this alias** in the GitLab settings; FR-021 requires it remain enabled for at least 90 days so external references with hard-coded `master` URLs continue to resolve. Blocks US1 (.gitlab-ci.yml uses `$CI_DEFAULT_BRANCH` resolving to `main`) and US3 in-repo sweep
