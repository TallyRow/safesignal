...
- [X] T018 [US3] Local clone update (one-time per maintainer workstation; after T002 GitLab default-branch rename): run `git fetch origin && git branch -m master main && git branch -u origin/main main && git remote set-head origin -a`. Verify `git branch --show-current` reads `main`. Document in CONTRIBUTING.md's "Local development setup" section (a small addition — fold into T022's CONTRIBUTING update)
