...
- [ ] T001 Capture pre-feature baselines into a new file `specs/005-cicd-pipeline/baselines.md`: (a) run `npm test` and record the headline counts (test files / passing / todo / failing / unhandled); (b) note current GitLab CI/CD usage from the project's Settings → Usage page (maintainer-side observation — record minutes-used-this-month for the comparison after T017's dogfood run). Use the same `## Pre-feature baseline` heading pattern as F003 + F004 baselines.md
...
- [ ] T026 [P] Capture post-feature test-suite baseline + bundle baselines for comparison: `npm test` should still produce 48 / 1088 / 10 / 0 / 0 (identical to T001 pre-feature baseline per FR-026); `gzip -c dist/index.mjs | wc -c` and `gzip -c dist/transport-beacon.mjs | wc -c` should produce sizes within ±1 KiB of the F003-established baselines (8162 B, 3101 B). Record into `baselines.md` (maps R-016 / FR-026 / SC-010)
