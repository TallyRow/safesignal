...
- [ ] T002 [P] Create beacon-network test doubles in `tests/helpers/beacon-network.ts`
...
- [ ] T003 Create subpath entry stub in `src/transport-beacon/index.ts`
...
- [ ] T009 [P] Add public-API contract test scaffolding in `tests/contract/transport-beacon.contract.test.ts`
...
- [ ] T017 [US1] Wire subpath exports in `src/transport-beacon/index.ts`
...
- [ ] T019 [US1] Validate quickstart against the host-app example
  Acceptance: The "Five-minute path (single application)" section of `quickstart.md` is executed by the host-app example exactly as written. The `quickstart.md` code blocks match the example's source (line-for-line where shown). Manual or scripted validation: copy-paste a quickstart code block into a vitest harness and assert it compiles + runs successfully under happy-dom.
